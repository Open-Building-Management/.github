...
I've tried to use reinforcement learning to create an energy building model called [EnergyGym](https://github.com/Open-Building-Management/EnergyGym) but it is difficult to train a portable model that adapts to various building types. In RL everything relies on a the (not easy) design of a reward, here a sparse reward that (tries to) balance confort and energy savings. The lack of real world classified datas led to use synthetic datas generated by a simple RC model and to practise off policy training like deep Q network. Results are not so bad on buildings with little isolation and some inertia. When trying to train a single agent on random types of buildings, things are really not convincing. 
